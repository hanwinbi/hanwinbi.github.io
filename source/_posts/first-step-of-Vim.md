...
第一次了解到 vim 是本科期间开设的 Linux 课程，当时对 Linux 的认识和兴趣不高，所以这门课最终也是和其他的课程一样被糊弄过去了，后来在看 MIT 公开课 [计算机教育中缺失的一课](https://missing-semester-cn.github.io/) 再一次和 vim 相遇，在完成基础的使用之后，由于疏于练习，并不是很熟练。近来复习相关内容的时候发现，vim 提供了相应的初学者教程——vimtutor。如果你想学习完整的内容，在安装 vim 之后在终端输入 `vimtutor` 即可进入相关的内容学习，以下是我将其中的内容拷贝以便复习。

## 基础使用
1. 光标在屏幕文本中的移动既可以用箭头键，也可以使用 `hjkl` 字母键。
	`h(左移)    j (下行)    k (上行)    l (右移)`
2. 欲进入 Vim 编辑器(从命令行提示符)，输入：vim 文件名 <回车>
